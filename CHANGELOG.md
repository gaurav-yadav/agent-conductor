...
### Added
- Experimental `codex` provider that launches the OpenAI Codex CLI with sandbox/search flags, environment isolation, and improved prompt/error detection.
...
- Updated conductor personas (core and CredCore supervisor) to confirm provider selection before launching workers, enabling smooth swaps between different backends.
...
- Release and persona-install milestones captured in `docs/backlog.md` (renamed from `docs/todo.md`).
